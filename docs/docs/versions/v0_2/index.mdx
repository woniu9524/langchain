...
LangChain v0.2 于 2024 年 5 月发布。此版本包含许多[破坏性更改和弃用项](/docs/versions/v0_2/deprecations)。本文档包含升级到 0.2.x 的指南。

:::note 参考

- [破坏性更改与弃用项](/docs/versions/v0_2/deprecations)
- [将旧版链迁移到 LCEL](/docs/versions/migrating_chains)
...
我们创建了一个工具来帮助迁移您的代码。该工具目前仍处于 **beta** 阶段，可能无法涵盖所有情况，但
我们希望它能帮助您更快地迁移代码。

迁移脚本具有以下限制：

1. 它仅限于帮助用户从旧导入迁移到新导入。它不处理其他弃用项。
2. 它无法处理涉及 `as` 的导入。
3. 新导入始终放置在全局作用域中，即使被替换的旧导入位于某个局部作用域中（例如，函数体内部）。
4. 它可能会遗漏一些已弃用的导入。
...
鉴于迁移脚本并非完美，您应该首先确保您的代码有备份（例如，使用 `git` 等版本控制）。
...
# 在包含 ipython 笔记本的代码上运行
# 应用所有导入更新，但排除从 langchain 到 langchain-core 的更新
