...
LangFair 是一个全面的 Python 库，专为对大型语言模型 (LLM) 用例进行偏见和公平性评估而设计。LangFair [代码库](https://github.com/cvs-health/langfair) 包含一个全面的框架，用于[选择偏见和公平性指标](https://github.com/cvs-health/langfair/tree/main#-choosing-bias-and-fairness-metrics-for-an-llm-use-case)，以及[演示笔记本](https://github.com/cvs-health/langfair/tree/main/examples) 和一份[技术手册](https://arxiv.org/abs/2407.10853)，其中讨论了 LLM 偏见和公平性风险、评估指标和最佳实践。

请访问我们的[文档网站](https://cvs-health.github.io/langfair/)，了解使用 LangFair 的详细说明。

## ⚡ 快速入门指南
### (可选) 为使用 LangFair 创建虚拟环境
我们建议在安装 LangFair 之前使用 venv 创建一个新的虚拟环境。为此，请按照[此处](https://docs.python.org/3/library/venv.html)的说明进行操作。
...
### 使用示例
以下代码示例说明了如何使用 LangFair 来评估文本生成和摘要用例中的偏见和公平性风险。以下示例假定用户已定义了用例中的提示列表 `prompts`。
...
```
我们可以使用 `ResponseGenerator.generate_responses` 为每个提示生成 25 个响应，这符合毒性评估的惯例。
```python
...
##### 计算毒性指标
可以使用 `ToxicityMetrics` 计算毒性指标。请注意，使用 `torch.device` 是可选的，如果可用 GPU 来加速毒性计算，则应使用它。
```python
...
为了简化文本生成和摘要用例的评估，`AutoEval` 类通过一个多步骤过程来完成，该过程仅用两行代码即可完成上述所有步骤。
```python
