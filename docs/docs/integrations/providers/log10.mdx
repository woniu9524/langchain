...
2. 将您的 `LOG10_TOKEN` 和 `LOG10_ORG_ID` 分别从“设置”和“组织”选项卡添加为环境变量。
3. 还要将 `LOG10_URL=https://log10.io` 和您常用的 LLM API 密钥（例如 `OPENAI_API_KEY` 或 `ANTHROPIC_API_KEY`）添加到您的环境变量中。
...
[更多详细信息 + 截图](https://log10.io/docs/observability/logs)，包括自托管日志的说明
