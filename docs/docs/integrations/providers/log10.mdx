...
此页面介绍如何在 LangChain 中使用 [Log10](https://log10.io)。

## 什么是 Log10？

Log10 是一个 [开源](https://github.com/log10-io/log10) 的无代理 LLM 数据管理和应用程序开发平台，可让您记录、调试和标记 Langchain 调用。

## 快速入门

1. 在 [log10.io](https://log10.io) 创建您的免费账户。
2. 将您的 `LOG10_TOKEN` 和 `LOG10_ORG_ID`（分别来自“设置”和“组织”选项卡）添加为环境变量。
3. 还请将 `LOG10_URL=https://log10.io` 和您常用的 LLM API 密钥（例如 `OPENAI_API_KEY` 或 `ANTHROPIC_API_KEY`）添加到您的环境中。
...
[更多详情 + 屏幕截图](https://log10.io/docs/observability/logs)，包括自托管日志的说明。
...
您也可以混合使用直接 OpenAI 调用和 Langchain LLM 调用：
