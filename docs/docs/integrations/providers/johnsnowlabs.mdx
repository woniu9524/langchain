...
通过开源的 `johnsnowlabs` 库，您可以访问 John Snow Labs 的企业级 NLP 库生态系统，该生态系统拥有超过 200 种语言的 21,000 多个企业级 NLP 模型。有关所有 24,000 多个模型，请参阅 [John Snow Labs 模型库](https://nlp.johnsnowlabs.com/models)。
...
您可以使用基于 `gpu`、`cpu`、`apple_silicon`、`aarch` 优化的二进制文件来嵌入您的查询和文档。
默认情况下使用 CPU 二进制文件。
启动会话后，您必须重启笔记本才能在 GPU 或 CPU 之间切换，否则更改将不会生效。
