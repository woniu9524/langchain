...
前往 [ModelScope](https://modelscope.cn/) 注册 ModelScope 并生成 [SDK token](https://modelscope.cn/my/myaccesstoken)。完成此操作后，设置 `MODELSCOPE_SDK_TOKEN` 环境变量：
...
`ModelScopeChatEndpoint` 类公开了来自 ModelScope 的聊天模型。在此处 [查看可用模型](https://www.modelscope.cn/docs/model-service/API-Inference/intro)。
