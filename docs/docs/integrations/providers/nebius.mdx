...
>[Nebius AI Studio](https://studio.nebius.ai/) 提供对各种尖端大型语言模型和嵌入模型的 API 访问，适用于多种用例。
...
要使用 Nebius AI Studio，您需要一个 API 密钥，可以从 [Nebius AI Studio](https://studio.nebius.ai/) 获取。API 密钥可以作为初始化参数 `api_key` 传递，或者设置为环境变量 `NEBIUS_API_KEY`。
...
支持的模型完整列表可在 [Nebius AI Studio 文档](https://studio.nebius.com/) 中找到。
...
查看 [使用示例](/docs/integrations/chat/nebius)。
...
查看 [使用示例](/docs/integrations/text_embedding/nebius)。
...
`NebiusRetriever` 利用 Nebius AI Studio 的嵌入，实现高效的相似性搜索。它利用高质量的嵌入模型，实现文档的语义搜索。

查看 [使用示例](/docs/integrations/retrievers/nebius)。
