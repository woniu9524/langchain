...
>[Outlines](https://github.com/dottxt-ai/outlines) 是一个用于受限语言生成的 Python 库。它提供了一个统一的接口来访问各种语言模型，并允许使用正则表达式匹配、类型约束、JSON schema 和上下文无关语法等技术进行结构化生成。
...
这些集成允许您将 Outlines 模型与 LangChain 一起使用，同时提供 LLM 和聊天模型接口。
