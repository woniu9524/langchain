...
```
参数：
1. url (str)：CnosDB 服务的 HTTP 连接主机名和端口号，不包含 "http://" 或 "https://"，默认为 "127.0.0.1:8902"。
2. user (str)：连接 CnosDB 服务使用的用户名，默认为 "root"。
3. password (str)：连接 CnosDB 服务用户的密码，默认为 ""。
4. tenant (str)：连接 CnosDB 服务使用的租户名称，默认为 "cnosdb"。
...
### SQL 数据库链
此示例演示了如何使用 SQL Chain 来回答关于 CnosDB 的问题。
```python
...
```
### SQL 数据库代理
此示例演示了如何使用 SQL 数据库代理来回答关于 CnosDB 的问题。
```python
