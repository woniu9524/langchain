...
2. user (str)：连接 CnosDB 服务时使用的用户名，默认值为 "root"。
3. password (str)：连接 CnosDB 服务时用户的密码，默认值为 ""。
4. tenant (str)：连接 CnosDB 时使用的租户名称，默认值为 "cnosdb"。
...
### SQL 数据库链
此示例演示了如何使用 SQL 链来回答有关 CnosDB 的问题。
```python
...
```
### SQL 数据库代理
此示例演示了如何使用 SQL 数据库代理来回答有关 CnosDB 的问题。
```python
