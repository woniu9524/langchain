...
本页介绍如何在 LangChain 中使用 [SearchApi](https://www.searchapi.io/) Google Search API。SearchApi 是一个实时 SERP API，可轻松抓取 SERP。
...
有一个名为 `SearchApiAPIWrapper` 的工具类封装了此 API。要导入此工具类：
