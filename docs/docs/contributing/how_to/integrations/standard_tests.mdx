...
在创建自定义类（无论是供自己使用还是发布到 LangChain 集成中）时，添加标准测试以确保其按预期工作非常重要。本指南将向您展示如何为每种集成类型添加标准测试。
...
如果您遵循了[上一指南](../package)，您应该已经安装了这些依赖项！
...
- [单元测试](../../../concepts/testing.mdx#unit-tests) (`langchain_tests.unit_tests`)：旨在用于隔离测试组件，无需访问外部服务。
- [集成测试](../../../concepts/testing.mdx#integration-tests) (`langchain_tests.integration_tests`)：旨在用于在可以访问外部服务（特别是组件旨在交互的外部服务）的情况下测试组件。

这两种类型的测试都实现为[`pytest` 基于类的测试套件](https://docs.pytest.org/en/7.1.x/getting-started.html#group-multiple-tests-in-a-class)。
...
在下面的选项卡中，我们展示了如何为每种组件类型实现标准测试：

<Tabs>

    <TabItem value="chat_models" label="Chat models">

要为聊天模型配置标准测试，我们继承 `ChatModelUnitTests` 和 `ChatModelIntegrationTests`。在每个子类上，我们覆盖以下 `@property` 方法来指定要测试的聊天模型和聊天模型的配置：
...
1. 如果模型旨在支持某个功能，则应通过测试；
2. 如果模型不支持某个功能，则应跳过该测试。

“可选”功能的测试通过可以覆盖测试模型子类上的一组属性来控制。

您可以在以下 API 参考中找到**所有可配置功能的列表**：
[单元测试](https://python.langchain.com/api_reference/standard_tests/unit_tests/langchain_tests.unit_tests.chat_models.ChatModelUnitTests.html)
和[集成测试](https://python.langchain.com/api_reference/standard_tests/integration_tests/langchain_tests.integration_tests.chat_models.ChatModelIntegrationTests.html)。
...
以下是如何为典型的向量存储配置标准测试（使用 `ParrotVectorStore` 作为占位符）：
...
| Fixture | Description |
| --- | --- |
| `vectorstore` | 一个生成器，用于为单元测试提供一个空的向量存储。向量存储在测试运行结束后会被清理。 |
...
请注意，在初始 `yield` 之前，我们使用[嵌入模型](/docs/concepts/embedding_models/)对象实例化了向量存储。这是一个预定义的“[假](https://python.langchain.com/api_reference/standard_tests/integration_tests/langchain_tests.integration_tests.vectorstores.VectorStoreIntegrationTests.html#langchain_tests.integration_tests.vectorstores.VectorStoreIntegrationTests.get_embeddings)嵌入模型”，它将为文档生成简短的、任意的向量。如果需要，您可以使用不同的嵌入对象。

在 `finally` 块中，我们调用了任何特定于集成的逻辑，以将向量存储带到干净的状态。此逻辑在每个测试之间执行（例如，即使测试失败）。
...
要为嵌入模型配置标准测试，我们继承 `EmbeddingsUnitTests` 和 `EmbeddingsIntegrationTests`。在每个子类上，我们覆盖以下 `@property` 方法来指定要测试的嵌入模型和嵌入模型的配置：
...
要为工具配置标准测试，我们继承 `ToolsUnitTests` 和 `ToolsIntegrationTests`。在每个子类上，我们覆盖以下 `@property` 方法来指定要测试的工具和工具的配置：

| Property | Description |
| --- | --- |
| `tool_constructor` | 要测试的工具的构造函数，或已实例化的工具。 |
| `tool_constructor_params` | 要传递给工具的参数（可选）。 |
| `tool_invoke_params_example` | 要传递给工具的 `invoke` 方法的参数示例。 |

如果您正在测试一个工具类并将 `MyTool` 等类传递给 `tool_constructor`，您可以在 `tool_constructor_params` 中传递构造函数的参数。

如果您正在测试一个已实例化的工具，您可以将已实例化的工具传递给 `tool_constructor`，并且不覆盖 `tool_constructor_params`。
